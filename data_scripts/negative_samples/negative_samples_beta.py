...
# %%
read_path = "../../data/customDatasets/beta_concatenated.tsv"
beta_df = pd.read_csv(read_path, sep="\t")
...
file_name = "negative_samples_beta_embeddings_dict.npz"
...
for i, epitope in enumerate(beta_df["Epitope"]):
    negative_epitope = search_negative_epitope_embedding(beta_df, i, epitope)
    negative_epitopes_cosine.append(negative_epitope)

# %%
print(len(negative_epitopes_cosine)) # should be: 229'862
...
# %%
beta_negative_epitope_df = beta_df.drop(["MHC A", "MHC B"], axis=1).copy(deep=True)
beta_negative_epitope_df["Epitope"] = epitopes
beta_negative_epitope_df["MHC A"] = mhc_a
beta_negative_epitope_df["MHC B"] = mhc_b
beta_negative_epitope_df["Binding"] = 0
beta_negative_epitope_df

# %%
beta_with_negative_df = pd.concat([beta_df.copy(deep=True), beta_negative_epitope_df], axis=0)

# %%
columns_to_ignore_for_duplicates = beta_with_negative_df.columns.difference(["TCR_name", "Binding"])
beta_with_negative_df.drop_duplicates(inplace=True, subset=columns_to_ignore_for_duplicates, keep="first")
beta_with_negative_df["TCR_name"] = range(1, len(beta_with_negative_df)+1)
beta_with_negative_df.reset_index(drop=True, inplace=True)

# %%
to_path = "../../data/customDatasets/negative_samples/"
file_name = "beta_concatenated_with_negative.tsv"

# %%
beta_with_negative_df.to_csv(to_path+"/"+file_name, sep="\t", index=False)
